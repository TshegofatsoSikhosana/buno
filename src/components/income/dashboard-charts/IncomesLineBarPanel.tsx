...
function IncomesLineBarPanel() {
  ...
  function getIncomes() {
    db.income.toArray().then((ex) => {
      ...
      const items = getItemsInOrder(ex).filter(e => Number(e.actualAmount)> 0);

      const itemLabels = new Set<string>();
      let filteredIncomes: IncomeItem[] = [];
      ...
      for (let index = 0; index < Array.from(itemLabels).length; index++) {
        const item = Array.from(itemLabels)[index];
                  const data = filteredIncomes.filter((e) => e.description === item );
...
export default IncomesLineBarPanel;
