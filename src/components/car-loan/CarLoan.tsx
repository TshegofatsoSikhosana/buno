...
// import BusinessItemEditForm from "./modals/BusinessItemEditForm";
...
function CarLoan(props: ExpensesProps) {
  ...
  const [filteredGoals, setFilteredGoals] = useState<BusinessItem[]>();
  const goalsService = new BusinessService();

  useEffect(() => {
    if (businesses) {
      const g = filterItems(filterType, businesses);
      setFilteredGoals([...g]);
    }
  ...
  async function getGoals() {
    const businesses = await goalsService.getBusinesses();
    setBusinesses([...businesses]);

    // const percentage = goalsService.getTotalContributionPercentage([...businesses]);
    // setTotalContributionPercentage(percentage);

    // const contributions = goalsService.getContributionsTotal([...businesses]);
    ...
    <div className="dashboard-container">
      <div className="w-100 p-5">
        ...
        <div className="p-2 mt-2">
            <div className="inline-block mr-5 w-1/12">
            <h1>Current Month </h1>
            <div>Operations</div>
          </div>
        ...
        <div className="p-2 mt-2">
          ...
          {openForm && (
            <BusinessItemEditForm
              ...
              refresh={getGoals}
              item={
                businesses && Number(selectedItem) >= 0
                  ? businesses[selectedItem - 1]
                  : undefined
              }
            />
          )}
        ...
        {filteredGoals?.length ? <>
            <div className="font-bold mt-2 inline-block" style={{fontSize: '22px'}}>
                Total Businesses: {filteredGoals ? filteredGoals.length : 0}
            </div>
            <div className="w-100 grid-flow-row mt-5 ">
              {filteredGoals?.map((business, index) => {
            ...
            {/* {goals.length && <div className="p-2">
                ...
                <div className="w-8/12 text-white inline-block p-2 mt-5">
                  <GoalsLineBarPanel BusinessItems={goals}/>
                </div>
                <div className="w-4/12 inline-block ">
                  <GoalsDoughnut BusinessItems={goals}/>
                </div>
              </div>
            } */}
